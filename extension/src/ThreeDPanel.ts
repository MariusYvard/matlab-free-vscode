...
export class ThreeDPanel {
    ...
    static show(id: string, rawPayload: ThreeDPayload): void {
        // Charge le JSON depuis le disque si nécessaire
    ...
    /** Envoie un message live (camlight, lighting, colormap…) au panneau actif. */
    static broadcast(msg: object): void {
        ThreeDPanel.activePanel?.vsPanel.webview.postMessage(msg)
    }

    /** Met à jour le titre du panneau actif. */
...
// ── Caméras (perspective + ortho, switchables) ────────────────────────────
...
// ── Axes helper (discret) ─────────────────────────────────────────────────
const axes = new THREE.AxesHelper(0) // taille ajustée après fitCamera
scene.add(axes)
...
const CMAPS = {
  ...
  viridis:t => { // approximation 4 stops
...
function buildPatch(p) {
  ...
  // Faces MATLAB 1-based → 0-based, triangles seulement (col 0-2)
  const fIdx = p.faces.map(f=>f.slice(0,3).map(i=>i-1)).flat()
  geo.setIndex(fIdx)

  // CData → couleurs par vertex
...
// ── Construction géométrie surf/mesh ──────────────────────────────────────
function buildSurf(p) {
  ...
  // Calcul zmin/zmax pour colormap
...
window.savePng = ()=>{
  // Rend une frame propre puis télécharge le canvas en PNG
  renderer.render(scene, camera)
  const a = document.createElement('a')
  a.download = '3d_figure.png'
...
window.toggleFullscreen = ()=>{
  ...
  if (isFullscreen) {
    ...
    wrap.style.zIndex = ''
    document.getElementById('cbar').style.display = ''
  }
